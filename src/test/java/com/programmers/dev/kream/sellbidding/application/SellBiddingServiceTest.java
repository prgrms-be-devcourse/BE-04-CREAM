...
class SellBiddingServiceTest {
    ...
    void getProductInformation() {
        ...
        // when
        ProductInformation productInformation = sellBiddingService.getProductInformation(product1.getName(), product1.getBrand().getName());

        // then
        assertAll(
                () -> assertThat(productInformation.productName()).isEqualTo(product1.getName()),
    ...
    @Test
    @DisplayName("판매입찰 페이지 조회시 잘못된 id로 조회할 경우 biddingSelectLines에는 정보가 없어야 한다")
    void getProductInformation_InvalidProductId() {
        ...
        ProductInformation productInformation = sellBiddingService.getProductInformation(product1.getName() + "alpha", product1.getBrand().getName());

        assertAll(
                () -> assertThat(productInformation.productName()).isEqualTo("Air Jordanalpha"),
                () -> assertThat(productInformation.biddingSelectLines().size()).isEqualTo(0)
        );
    ...
    void saveSellBidding() {
        ...
        SellBiddingRequest sellBiddingRequest = new SellBiddingRequest(190000, 30L);
         ...
         // when
        SellBiddingResponse sellBiddingResponse = sellBiddingService.saveSellBidding(user.getId(), product.getId(), sellBiddingRequest);
    ...
    void saveSellBiddingInvalidSizedProductId() {
        ...
        Product product = makeProduct("adidas", "푸마", 300);
        SellBiddingRequest sellBiddingRequest = new SellBiddingRequest(190000, 30L);

        // when && then
        assertThatThrownBy(
                () -> sellBiddingService.saveSellBidding(user.getId(), product.getId() + 1, sellBiddingRequest)
        ).isInstanceOf(CreamException.class);

        assertThatThrownBy(
                () -> sellBiddingService.saveSellBidding(user.getId() + 1, product.getId(), sellBiddingRequest)
        ).isInstanceOf(CreamException.class);
    ...
    void transactPurchaseBidding() {
        ...
        Product product = makeProduct("Nike", "air jordan", 260);
